        ...
        if ($_REQUEST["AncienPassword"] == $utilisateur["motDePasse"]) {
            ...
            if ($_REQUEST["NouveauPassword"] == $_REQUEST["ConfirmPassword"]) {
                if (CalculComplexiteMdp($_REQUEST["NouveauPassword"]) >= 90) {
                    ...
                    $Vue->setEntete(new Vue_Structure_Entete());
                    $Vue->setMenu(new Vue_Menu_Administration($typeConnexion));
                    $Vue->addToCorps(new Vue_Utilisateur_Changement_MDP("<label><b>Le mot de passe n'est pas assez complexes</b></label>", "Gerer_monCompte"));
                }
            } else {
                $Vue->setEntete(new Vue_Structure_Entete());
                $Vue->setMenu(new Vue_Menu_Administration($typeConnexion));
                $Vue->addToCorps(new Vue_Utilisateur_Changement_MDP("<label><b>Le mot de passe n'est pas identique</b></label>", "Gerer_monCompte"));
            }
        } else {
            $Vue->setEntete(new Vue_Structure_Entete());
            $Vue->setMenu(new Vue_Menu_Administration($typeConnexion));
